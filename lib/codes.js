...
exports.create = function( type, text, options )
{
	...
	var asCanvas = opts.asCanvas || false;

	if ( opts.hasOwnProperty( 'asCanvas' ) ) {
		delete opts.asCanvas;
	}
	...
	console.log('rescode: returning rotated barcode, ' + ( asCanvas ? 'as node canvas' : 'as dataURL') + '.');
	return bw.bitmap().create(rot, asCanvas);
};
